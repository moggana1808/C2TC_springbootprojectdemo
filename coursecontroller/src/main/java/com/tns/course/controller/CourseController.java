...
public class CourseController {
    ...
    @GetMapping("/all")
    public ResponseEntity<Page> findAllCourses(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        try {
            PageRequest pageable = PageRequest.of(page, size);
            Page coursesPage = courseService.findAll(pageable);
  ...
  //End point to retrieve all courses by limit in ascending order.
    ...
    public ResponseEntity<Page> findByTitleContaining(
            @RequestParam String keyword,
            @RequestParam int page,
            @RequestParam int size) {
        try {
            Pageable pageable = PageRequest.of(page, size);
            Page coursesPage = courseService.findByTitleContaining(keyword, pageable);
            return ResponseEntity.ok(coursesPage);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
        }
